    ...
    cGenerateView.prototype = {
        ...
        renderCell: function(objP) {
            var self = this,
                ...
                rowprop, freezeCols = o.freezeCols,
            ...
            if (Export) {
                ...
                dstyle && style.push(styleStr(dstyle));
                dprop = dprop || {};
...
(function($) {
    ...
    _pq.cFormula = function(that) {
        ...
        that.one("ready", function() {
            that.on("CMInit", self.onCMInit.bind(self))
        }).on("dataAvailable", self.onDA.bind(self)).on(true, "change", self.onChange.bind(self))
    };
...
(function($) {
    pq.excelImport = {
        ...
        getCsv: function(text) {
            ...
            separator = new RegExp(text.indexOf("	") == -1 ? "," : "	", "g");
            return {
                sheets: [{
                    rows: text.split(/\r\n|\r|\n/g).map(function(row) {
                        return {
        ...
        Import: function(obj, fn) {
            var self = this,
                ...
                cb = function(data, type2) {
                    fn(self[csv ? "getCsv" : "getWorkBook"](data, obj.type || type2, obj.sheets))
                };
            if (url) {
                url += "?" + Math.random();
    ...
    cExport.prototype = $.extend({
        ...
        getXlsBody: function(CM, CMLen, data, dataLen, remotePage, offset, iMerge, render, iGV, shiftR, rowInit) {
            var self = this,
                ...
                cells, rowData, ri, rip, di, colStyle, colProp, row, cell, cellattr, cellattrdi, tmp, cellprop, cellpropdi, cellstyle, cellstyledi, shiftRC = _pq.cFormulas.shiftRC(that),
                format;
            for (i = 0; i < dataLen; i++) {
                rowData = data[i];
                ...
                self.copyStyle(row, rowData.pq_rowstyle, rowData.pq_rowprop);
                if (rowInit) {
    ...
    $.extend(cF, {
        deString: function(fn, cb, exec) {
            ...
            fn = fn.replace(/(?:^|[^"]+)"(([^"]|"{2})+)"(?=([^"]+|$))/g, function(a, b) {
                var indx = a.indexOf(b);
                arr.push(b);
                return a.slice(0, indx - 1) + "#" + (arr.length - 1) + "#"
            });
            fn = cb(fn);
...
(function($) {
    ...
    _pq.cEditor = function cEditor(that) {
        ...
        that.on("editorBeginDone", function(evt, ui) {
            ui.$td[0].edited = true;
            self.fixWidth(ui);
            var edInit = (ui.column.editor || {}).init;
