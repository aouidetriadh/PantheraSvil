...
public class BudgetCommessa extends BudgetCommessaPO {
   ...
   public void writeCommessaGruppo(JSONArray data, String id, String parentId, String descCommessa,
      ...
      json.put("BudVal", getValoreString(dettaglio.getCostoTotale(), true));
      if (isATempo) 
         json.put("BudHH", getOreString(dettaglio.getTempoTotale(), dettaglio.getComponenteCosto().isGestioneATempo(), true));
   ...
   public void writeComponentePropri(JSONArray data, String id, String parentId, boolean isATempo, boolean isDettaglio, BudgetCommessaDet dettaglio){
      try {
         ...
         json.put("BudVal", getValoreString(dettaglio.getCostoLivello(), true));
         if (isATempo) 
            json.put("BudHH", getOreString(dettaglio.getTempoLivello(), dettaglio.getComponenteCosto().isGestioneATempo(), true));

         json.put("pq_rowcls", stileRiga);

         JSONObject stiliCelle = new JSONObject();
         if(valoreEditabile)
            stiliCelle.put("BudVal", "CellaEditabile");
         if(oraEditabile)
            stiliCelle.put("BudHH", "CellaEditabile");
         if(cmpTotali)
