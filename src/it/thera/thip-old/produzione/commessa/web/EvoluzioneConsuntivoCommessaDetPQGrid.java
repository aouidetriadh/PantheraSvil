...
/**
 * EvoluzioneConsuntivoCommessaDetPQGrid
 ...
 */
public class EvoluzioneConsuntivoCommessaDetPQGrid extends WebParamQuery{
	
	protected EvoluzioneConsuntivoCommessa iEvoluzioneConsuntivo = null;
	
	public EvoluzioneConsuntivoCommessaDetPQGrid(ServletEnvironment se) {
		setServletEnvironment(se);
		setName("grid_EvoluzioneConsuntivoCommessaDet");
		initEvoluzioneConsuntivoCommessa(se);
	}
	
	public EvoluzioneConsuntivoCommessa getEvoluzioneConsuntivoCommessa() {
		return iEvoluzioneConsuntivo; 
	}
	
	public void setEvoluzioneConsuntivoCommessa(EvoluzioneConsuntivoCommessa evoluzioneConsuntivo) {
		iEvoluzioneConsuntivo = evoluzioneConsuntivo;
	}
	
	public void initEvoluzioneConsuntivoCommessa(ServletEnvironment se){	
		EvoluzioneConsuntivoCommessa evoluzioneConsuntivo = null;
	    ...
	    String dataInizioStr = getServletEnvironment().getRequest().getParameter("DataInizio");
	    String dataFineStr = getServletEnvironment().getRequest().getParameter("DataFine");
	    ...
	    evoluzioneConsuntivo = (EvoluzioneConsuntivoCommessa)Factory.createObject(EvoluzioneConsuntivoCommessa.class);
		
		evoluzioneConsuntivo.setIdAzienda(idAzienda);
		evoluzioneConsuntivo.setIdCommessa(idCommessa);
		evoluzioneConsuntivo.setDataInizio(dataInizio);
		evoluzioneConsuntivo.setDataFine(dataFine);
		evoluzioneConsuntivo.setTotali(totali);
		evoluzioneConsuntivo.setDettagliCommessa(dettagliCommessa);
		evoluzioneConsuntivo.setDettagliSottoCommesse(dettagliSottoCommesse);
		evoluzioneConsuntivo.setComponentiPropri(componentiPropri);
		evoluzioneConsuntivo.setSoloComponentiValorizzate(soloComponentiValorizzate);
		setEvoluzioneConsuntivoCommessa(evoluzioneConsuntivo);	    	    
	}
	
   public void writeImport(PrintWriter out) throws IOException, JSONException {
      super.writeImport(out);
      out.println(com.thera.thermfw.web.WebJSTypeList.getImportForJSLibrary("it/thera/thip/produzione/commessa/EvoluzioneConsuntivoCommessa.js",  getServletEnvironment().getRequest()));
   }
	...
	protected JSONObject getData() throws JSONException {
      ...
      data.put("data", iEvoluzioneConsuntivo.getDataModel());
      data.put("TotHH", "");
      data.put("TotVal", "");
      return data;
	}
	...
	protected JSONArray getColumnModel() throws JSONException {
		return getEvoluzioneConsuntivoCommessa().getColumnsDescriptors();
	}
	...
	public List check() {
		...
		boDC.initialize("EvoluzioneConsunCommessa");
		boDC.setBo(getEvoluzioneConsuntivoCommessa());
		boDC.check();
