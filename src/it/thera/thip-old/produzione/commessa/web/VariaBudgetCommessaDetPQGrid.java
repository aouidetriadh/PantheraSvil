   ...
   public void writeImport(PrintWriter out) throws IOException, JSONException 
   {
	...
	public void write(PrintWriter out) throws IOException, JSONException {		
		...
		if(getVariaBudgetCommessa() != null && getVariaBudgetCommessa().getCommessa() != null) {
            ...
            out.println("$" + getName() + ".pqGrid(\"instance\").colModel[" + start + "].render=nuovoBudgetCellRender;");
            out.println("$" + getName() + ".pqGrid(\"instance\").colModel[" + (start - 1) + "].render=nuovoBudgetCellRender;");
            ...
            out.println("$" + getName() + ".pqGrid(\"instance\").colModel[" + (start - 2) + "].render=editableColumnValCellRender;");
            out.println("$" + getName() + ".pqGrid(\"instance\").colModel[" + (start - 3) + "].render=editableColumnValCellRender;");
            out.println("$" + getName() + ".pqGrid(\"instance\").colModel[" + (start - 4) + "].render=editableColumnOreCellRender;");
            
            /*
				if(getVariaBudgetCommessa().getConsuntivoCommessa() != null) {
					out.println("$" + getName() + ".pqGrid(\"instance\").colModel[" + cols + "].render=nuovoBudgetCellRender;");
					out.println("$" + getName() + ".pqGrid(\"instance\").colModel[" + (cols - 1) + "].render=nuovoBudgetCellRender;");
					out.println("$" + getName() + ".pqGrid(\"instance\").colModel[" + (cols - 2) + "].editable=columnValoreEditable;");
					out.println("$" + getName() + ".pqGrid(\"instance\").colModel[" + (cols - 3) + "].editable=columnValoreEditable;");
					out.println("$" + getName() + ".pqGrid(\"instance\").colModel[" + (cols - 4) + "].editable=columnOreEditable;");
				}
				else {
					out.println("$" + getName() + ".pqGrid(\"instance\").colModel[7].render=nuovoBudgetCellRender;");
					out.println("$" + getName() + ".pqGrid(\"instance\").colModel[6].render=nuovoBudgetCellRender;");
					out.println("$" + getName() + ".pqGrid(\"instance\").colModel[5].editable=columnValoreEditable;");
					out.println("$" + getName() + ".pqGrid(\"instance\").colModel[4].editable=columnValoreEditable;");
					out.println("$" + getName() + ".pqGrid(\"instance\").colModel[3].editable=columnOreEditable;");
				}
				*/
			}
			else {
            ...
            out.println("$" + getName() + ".pqGrid(\"instance\").colModel[" + (start - 2) + "].editable=columnValoreEditable;");
            out.println("$" + getName() + ".pqGrid(\"instance\").colModel[" + (start - 1) + "].render=editableColumnValCellRender;");
            out.println("$" + getName() + ".pqGrid(\"instance\").colModel[" + (start - 2) + "].render=editableColumnValCellRender;");

            /*
				if(getVariaBudgetCommessa().getConsuntivoCommessa() != null) {
					out.println("$" + getName() + ".pqGrid(\"instance\").colModel[" + cols + "].render=nuovoBudgetCellRender;");
					out.println("$" + getName() + ".pqGrid(\"instance\").colModel[" + (cols - 1) + "].editable=columnValoreEditable;");
					out.println("$" + getName() + ".pqGrid(\"instance\").colModel[" + (cols - 2) + "].editable=columnValoreEditable;");
				}
				else {
					out.println("$" + getName() + ".pqGrid(\"instance\").colModel[4].render=nuovoBudgetCellRender;");
					out.println("$" + getName() + ".pqGrid(\"instance\").colModel[3].editable=columnValoreEditable;");
					out.println("$" + getName() + ".pqGrid(\"instance\").colModel[2].editable=columnValoreEditable;");
				}
				*/
			}	
			//out.println("$" + getName() + ".pqGrid(\"instance\").on(\"change\", cellSaveVariaBudget)");
	...
	protected JSONObject getData() throws JSONException {
      ...
      return data;
	   /*
		JSONObject data = new JSONObject();
		String key = servletEnvironment.getRequest().getParameter("key");
		String action = servletEnvironment.getRequest().getParameter("myAction");
		String idAzienda = servletEnvironment.getRequest().getParameter("IdAzienda");
		String idCommessa = servletEnvironment.getRequest().getParameter("IdCommessa");		
	    String dataRiferimentoStr = servletEnvironment.getRequest().getParameter("DataRiferimento");
	    char statoAvanzamento = BaseServlet.getStringParameter(servletEnvironment.getRequest(), "StatoAvanzamento").charAt(0);	    
	    boolean totali = BaseServlet.getBooleanParameter(servletEnvironment.getRequest(), "Totali");
		boolean dettagliCommessa = BaseServlet.getBooleanParameter(servletEnvironment.getRequest(), "DettagliCommessa");
		boolean dettagliSottoCommesse = BaseServlet.getBooleanParameter(servletEnvironment.getRequest(), "DettagliSottoCommesse");
		boolean componentiPropri = BaseServlet.getBooleanParameter(servletEnvironment.getRequest(), "ComponentiPropri");
		boolean soloComponentiValorizzate = BaseServlet.getBooleanParameter(servletEnvironment.getRequest(), "SoloComponentiValorizzate");
	    
		String url = "/" +	IniFile.getValue("thermfw.ini", "Web", "WebApplicationPath") + "/" + IniFile.getValue("thermfw.ini", "Web", "ServletPath") + 
					"/it.thera.thip.produzione.commessa.web.VariaBudgetCommessaDetServlet?myAction=" + action + "&key=" + key;
		String params = "&IdAzienda=" + idAzienda + "&IdCommessa=" + idCommessa  
				        + "&DataRiferimento=" + dataRiferimentoStr + "&StatoAvanzamento=" + statoAvanzamento;
		params += "&Totali=" + totali + "&DettagliCommessa=" + dettagliCommessa + "&DettagliSottoCommesse=" + dettagliSottoCommesse + "&ComponentiPropri=" + componentiPropri + "&SoloComponentiValorizzate=" + soloComponentiValorizzate;
		
		data.put("location", "remote");
		data.put("sorting", "local");
		data.put("url", url+params); 
		data.put("method", "POST");
		data.put("recIndx", "key");
