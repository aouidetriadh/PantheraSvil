   ...
   public boolean creaNuovoBudgetCommessa() {
	...
	public ErrorMessage checkDelete() {
	   ...
	   if(esisteVariazioneSuccessiva)
	      //return new ErrorMessage("FORMULA001", "La variazione di budget non eliminabile perchè non è l'ultima");
		   return new ErrorMessage("THIP_TN911");
	   else if(getStatoAvanzamento() == DEFINITIVO)
         //return new ErrorMessage("FORMULA001", "La variazione di budget non eliminabile perchè definitiva");
		   return new ErrorMessage("THIP_TN912");
	   return null;
	}
	...
	public JSONArray getColumnModelATempo(String title, boolean editable) throws JSONException {
		...
		else if(title.equals("NuovoBudget")) {
			columnOre.put("style", new JSONObject().put("color", "blue"));
		}
		else {
	...
	public void writeCommessaGruppo(JSONArray data, String id, String parentId, String descCommessa,
		...
		boolean valValorizzato = dettaglioTotali.getCostoTotale().compareTo(ZERO) != 0;
		json.put("BudVal", "" + budgetDet.getCostoTotale());
		json.put("VarVal", "" + (valValorizzato? dettaglioTotali.getCostoTotale() : ""));
		BigDecimal nuovoVal = budgetDet.getCostoTotale();
  	   if(valValorizzato)
  		   nuovoVal = sum(budgetDet.getCostoTotale(), dettaglioTotali.getCostoTotale());
  	   json.put("NuoVal", "" + nuovoVal);
  	   json.put("Not", getNote());
		
  	   if(isATempo) {
  		   if(dettaglioTotali.getComponenteCosto().isGestioneATempo()) {
  			   boolean oreValorizzate = dettaglioTotali.getTempoTotale().compareTo(ZERO) != 0;
  			   json.put("BudHH", "" + budgetDet.getTempoTotale());
  			   json.put("VarHH", "" + (oreValorizzate? dettaglioTotali.getTempoTotale() : ""));
  			   BigDecimal nuovoHH = budgetDet.getTempoTotale();
  			   if(oreValorizzate)
  				   nuovoHH = sum(budgetDet.getTempoTotale(), dettaglioTotali.getTempoTotale());
  			   json.put("NuoHH", "" + nuovoHH);
			}
			else {
				json.put("BudHH", "");
				json.put("VarHH", "");
	...
	public void writeTotale(JSONArray data, String id, String idParent, VariaBudgetCommessaDet dettaglio, boolean isATempo, boolean isDettaglio, BudgetCommessaDet budgetDet, ConsuntivoCommessaDet consDet) {
		try {
			...
			json.put("BudVal", "" + budgetDet.getCostoTotale());
			json.put("VarVal", "" + (valValorizzato? dettaglio.getCostoTotale() : ""));
			BigDecimal nuovoVal = budgetDet.getCostoTotale();
			if(valValorizzato)
				nuovoVal = sum(budgetDet.getCostoTotale(), dettaglio.getCostoTotale());
			json.put("NuoVal", "" + nuovoVal);
			...
			if(isATempo) {
				if(dettaglio.getComponenteCosto().isGestioneATempo()) {
					...
					json.put("BudHH", "" + budgetDet.getTempoTotale());
					json.put("VarHH", "" + (oreValorizzate? dettaglio.getTempoTotale() : ""));
					BigDecimal nuovoHH = budgetDet.getTempoTotale();
					if(oreValorizzate)
						nuovoHH = sum(budgetDet.getTempoTotale(), dettaglio.getTempoTotale());
					json.put("NuoHH", "" + nuovoHH);
				}
				else {
					json.put("BudHH", "");
	...
	public void writeComponentePropriDettagli(JSONArray data, String id, String idParent, List<VariaBudgetCommessaDet> varsCommessaDet, Commessa commessa, boolean isATempo, boolean soloComponentiValorizzate) {
		...
		for(int i = 0; i < varsCommessaDet.size(); i++) {
			...
			boolean budgetValorizzato = budgetDet != null && budgetDet.getCostoLivello() != null && budgetDet.getCostoLivello().compareTo(new BigDecimal(0)) != 0;
	...
	public void writeComponentePropri(JSONArray data, String id, String idParent, VariaBudgetCommessaDet dettaglio, boolean isATempo, boolean isDettaglio, BudgetCommessaDet budgetDet, ConsuntivoCommessaDet consDet) {
		try {
			...
			json.put("VarVal", "" + (valValorizzato? dettaglio.getCostoLivello() : ""));
			BigDecimal nuovoVal = budgetDet.getCostoLivello();
			if(valValorizzato)
				nuovoVal = sum(budgetDet.getCostoLivello(), dettaglio.getCostoLivello());
			json.put("NuoVal", "" + nuovoVal);
			...
			if(isATempo) {
				if(dettaglio.getComponenteCosto().isGestioneATempo()) {
					...
					json.put("VarHH", "" + (oreValorizzate? dettaglio.getTempoLivello() : ""));
					BigDecimal nuovoHH = budgetDet.getTempoLivello();
					if(oreValorizzate)
					   nuovoHH = sum(budgetDet.getTempoLivello(), dettaglio.getTempoLivello());
					json.put("NuoHH", "" + nuovoHH);
				}
				else {
					json.put("BudHH", "");
